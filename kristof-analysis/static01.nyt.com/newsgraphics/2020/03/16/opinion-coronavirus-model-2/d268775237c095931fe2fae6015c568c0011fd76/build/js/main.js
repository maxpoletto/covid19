...
/* -
ICU bed max Y scale not working
can tooltip be faded in and out, should it be?
mobile slider beneath chart
mobile slider one at a time with next button clearer?
resize
- */
...
function makeViz(){
  ...
  // Build initial run
  processData();
  xDate.domain([startDate, max(data, d => new Date(d.Time))]);
  x.domain([0, data.length]);
  xAxis.call(axisBottom(xDate).tickFormat(xFormat).ticks(4));

  var barWidth = (width / (data.length))+1;
  function update(){
    processData();
    var yMax = max([max(data, d => d.tot),37800000]);
    yInvert.domain([0, yMax]);
    yAxis.call(yAxisCreator);
    updateText();
    updatePaths();
    jquery('.model-wrapper.single .model-target').html(jquery('.model-wrapper.full .model-target').html());
  }

  function updateText(){
    jquery('.live').each(function(){
      var output = '';
      switch(jquery(this).attr('class').replace('live ','')){
        case 'infections':
          var output = totalYFormat(totals.infections);
          break;
        case 'dead':
          var output = totalYFormat(totals.dead);
          break;
        case 'location':
          var output = ' ' + location;
          break;
        case 'recoveries':
          var output = totalYFormat(totals.recoveries);
          break
        case 'infections-peak':
          var output = totalYFormat(totals['infections-peak']);
          break;
        case 'infections-peak-date':
          var output = moment(totals['infections-peak-date'].Time).format('MMMM D');
          break;
      }
      if(output != '') jquery(this).text(output);
    });
    if(totals.infections < 1000){
      jquery('.unrealistic').show();
    } else {
      jquery('.unrealistic').hide();
    }
  }

  function updatePaths(){

    barContainer.data(stack$1);
    bars.data(d => d);

    bars.attr('height', function(d,i) {
      return yInvert(d[0])-yInvert(d[1])
    })
    .attr('y', function(d) {
      return yInvert(d[1])
    });
    ...
    interventionBar
      .attr('transform', `translate(${(width*(InterventionTime/(overallN*2)))}, 0)`);
    interventionBarText
      .attr('transform', `translate(${(width*(InterventionTime/(overallN*2)))}, -5)`);
    interventionActive
      .text(duration + ' days of interventions');
    interventionBar.select('rect.intervention-bg')
      .attr('width', d=> {
        return xDate(new Date(moment(startDate).add(duration, 'd').format('YYYY/MM/DD')))});

    // var top = margin.top+yInvert((totals['infections-peak-date'].Infected + totals['infections-peak-date'].Dead + totals['infections-peak-date'].Hospital)) //+ totals['infections-peak-date'].Exposed
    // var left = margin.left+Math.round(xDate(totals['infections-peak-date'].Time))
    // if(left > width*.75){
    //   var classed = 'active-text align-right'
    // } else {
    //   var classed = 'active-text'
    // }
    // var text = totalYFormat(totals['infections']) + '<br />total infections'
    // var text = ''+totalYFormat((totals['infections-peak-date'].Infected + totals['infections-peak-date'].Hospital)) + '<br />infections at peak' //totals['infections-peak-date'].Exposed +
    // $('.active-text').attr('class', classed);
    // var toptop = isMobile ? top > height * .75 ? (top-(top*.2)) : top > height*.75 ? (top-(top*.1)) : top : top
    // $('.model-wrapper.full .active-text').css({
    //   left: left+'px',
    //   top: toptop+'px'
    // })
    // top = isMobile ? top > height * .75 ? (top-(top*.2)) : top : top > height*.75 ? (top-(top*.1))*.75 : top // < height/2 ? top * 1.1 :
    // top = top < height/2 ? top : top-(top*.25)
    // $('.model-wrapper.single .active-text')
    // .css({
    //   // left: (margin.left+15)+'px',
    //   top: (margin.top*.75)+'px'
    // })
    //   .html(text)
    // activeTextSpan
    //   .html(text)
  ...
  // Build intervetion vertical bar
  var interventionBar = svg.append('g')
    .attr('transform', `translate(${width}, 0)`)
    .attr('class', 'intervention-bar');
  interventionBar.append('rect')
    .attr('class', 'intervention-bg')
    .attr('width', xDate(moment(startDate).add(duration, 'd').format('YYYY/MM/DD')))
    .attr('height', height)
    .attr('fill', '#F3F3F3');

  // Create container for the data
  var barContainer = svg.selectAll('g.bar-container')
    .data(stack$1).enter()
  .append('g')
    .attr('class', d => d[0].key);

  var interventionBarText = svg.append('g')
    .attr('transform', `translate(${width}, -25)`)
    .attr('class', 'intervention-bar-text');
  var interventionActive = interventionBarText.append('text')
      .attr('fill', 'rgba(0, 0, 0, 0.5)')
      .attr('y', 0)
      .attr('x', 0)
      .attr('fill', 'rgba(99, 98, 67, 0.5)')
      .text('Active')
      .attr('class', 'chart-label intervention-label');

  // Create the bars for the data
  ...
  // var activeText = d3.selectAll('.model-wrapper.full .active-text')
  // var activeTextSpan = activeText.select('span')
  //   .text(totals.infections)

  // Build the today bar
  var todayBar = svg.append('g')
    .attr('transform', `translate(${xDate(new Date())}, ${height-6}) `)
    .attr('opacity', .5);
    todayBar.append('rect')
    .attr('width', .75)
    .attr('height', 6)
    .attr('fill', 'rgba(0, 0, 0, 0.98)')
    .attr('x', -.75/2)
    .attr('y', 0);
  todayBar.append('text')
    .text('Today')
    .attr('fill', 'rgba(0, 0, 0, 0.98)')
    .style('opacity', 1)
    .attr('class', 'chart-label today-label')
    .attr('y', -5);
  ...
  function inactiveSpan(n){
    jquery(n).removeClass('active');
  }
  var interventionScale = (n) => {
    return n > .6 ? 'Aggressive' : n > .4 ? 'Moderate' : n > .2 ? 'Mild' : n > 0 ? 'Few' : 'None'
  };
  var warmScale = (n) => {
    return n > .66 ? 'High' : n > .33 ? 'Medium' : n < .05 ? 'None' : 'Low'
  };
  jquery('.slider').each(function(){
    ...
    jquery(this).find('.button').click(function(){
      ...
      N = parseFloat(jquery(this).data('val'))*.7;
      if(N > usPop){
        // Hide seasonal
        location = 'world';
        jquery('*[data-val="seasonal_effect"]').addClass('inactive');
      } else {
        location = 'United States';
        jquery('*[data-val="seasonal_effect"]').removeClass('inactive');
        seasonal_effect = old_seasonal_effect;
      }
      update();
    });
    // Establish slider

    var slider = document.querySelector('#' + id +' input');
    if(slider){
      slider.addEventListener('mousedown', function(){
        activeSpan('#' +val + ' span.live');
      });
      slider.addEventListener('mouseup', function(){
        inactiveSpan('#' +val + ' span.live');
      });
      slider.addEventListener('input', function(){
        if(jquery('#'+id).find('.drag-me').hasClass('active')) {
          jquery('#'+id).find('.drag-me').removeClass('active');
        }
        var sliderValue = parseFloat(slider.value);
        switch(val){
          case 'R0':
            R0 = sliderValue;
            var output = sliderValue.toFixed(1);
          break;
          case 'CFR':
            CFR = sliderValue;
            var output = (sliderValue*100).toFixed(1)+'%';
          break;
          case 'InterventionAmt':
            InterventionAmt = 1-sliderValue;
            var output = interventionScale(sliderValue);//Math.round(sliderValue*100)+'%'
            break;
          case 'InterventionTime':
            InterventionTime = sliderValue;
            var output = moment(startDate).add(sliderValue, 'd').format('MMMM D');
                output = moment(new Date()).format('MMMM D') == output ? 'Today' : output;
            break;
          case 'D_incbation':
            D_incbation = sliderValue;
            var output = sliderValue.toFixed(1);
            break;
          case 'P_SEVERE':
            P_SEVERE = sliderValue;
            var output = (sliderValue*100).toFixed(1)+'%';
            break;
          case 'seasonal_effect':
            seasonal_effect = sliderValue;
            old_seasonal_effect = sliderValue;
            var output = warmScale(sliderValue);
            break;
          case 'duration':
            duration = sliderValue;
            var output = Math.round(sliderValue) + ' days';
            break;
        }
        jquery('[data-val="'+val + '"] .live').text(output);
        jquery('[data-val="'+val + '"] input').val(sliderValue);
        update();
      });
    }
  });
